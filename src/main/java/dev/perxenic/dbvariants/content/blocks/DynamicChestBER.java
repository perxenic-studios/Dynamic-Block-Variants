...
public class DynamicChestBER implements BlockEntityRenderer<DynamicChestBlockEntity> {
    ...
    public void render(@NotNull DynamicChestBlockEntity blockEntity, float partialTick, @NotNull PoseStack stack,
        ...
        if (blockEntity.dynamicMaterial == null) {
            DBVariants.LOGGER.error("Dynamic Chest at {} attempted to render but dynamic material was null", blockEntity.getBlockPos());
            return;
        }

        ItemStack itemStack = new ItemStack(BuiltInRegistries.ITEM.get(blockEntity.dynamicMaterial));
